...
Descripción general:
- Lee valores de temperatura y LDR durante `duration_seconds` segundos con un intervalo
  `sample_interval_s`.
- Almacena lecturas crudas en memoria como lista de tuplas (timestamp_iso, temp, ldr).
- Guarda resultados en results/raw_readings.csv y metadata en results/metadata.json.
- Genera una gráfica results/plot.png con ambas series (temperatura y LDR).
- Implementa escritura atómica del CSV (archivo temporal -> renombrado).
- Tiene manejo básico de errores y logging simple.
"""
...
def simulate_reading(t_seconds):
    ...
    ldr_base = 400 + 50 * (0.5 * (1 + random.sin(t_seconds / 5.0)))
...
def acquire_data(mode="sim", port=None, baud=115200, duration_seconds=DEFAULT_DURATION, sample_interval_s=DEFAULT_INTERVAL):
    ...
    # Si se quisiera implementar lectura serial real, se haría aquí con pyserial.
    # Para mantener el ejemplo autocontenido, la función serial no está implementada
    # completamente; en modo 'serial' se intenta abrir puerto si pyserial está disponible.
    serial_obj = None
    if mode == "serial":
        try:
            import serial
        ...
        while elapsed < duration_seconds:
            ...
            try:
                if mode == "sim":
                    ...
                    # modo serial: leer línea y parsear
                ...
                # En caso de error, registrar NaN o valores sentinel
                temp, ldr = float("nan"), -1

            readings.append((t_now, temp, ldr))
            sample_count += 1

            # esperar hasta la siguiente muestra (intentar respetar intervalo)
...
def save_csv(readings, csv_path):
    ...
    for ts, temp, ldr in readings:
        # temp puede ser NaN; ldr puede ser -1 en error
